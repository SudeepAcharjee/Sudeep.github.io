...
import { parseISO, getDay, subWeeks, nextDay, differenceInCalendarDays, eachDayOfInterval, formatISO, getMonth, getYear } from 'date-fns';
...
const ActivityCalendar = ({
  ...
  function renderCalendar() {
    return weeks.map((week, weekIndex) => week.map((activity, dayIndex) => {
      ...
      const block = /*#__PURE__*/React.createElement("rect", _extends({}, getEventHandlers(activity), {
        x: 0,
  ...
  function renderFooter() {
    ...
    }, labels.totalCount ? labels.totalCount.replace('{{count}}', String(totalCount)).replace('{{year}}', String(year)) : `${totalCount} activities in ${year}`), !loading && !hideColorLegend && /*#__PURE__*/React.createElement("div", {
      className: getClassName('legend-colors', styles.legendColors)
    }, /*#__PURE__*/React.createElement("span", {
      style: {
        marginRight: '0.4em'
      }
    }, labels?.legend?.less ?? 'Less'), Array(maxLevel + 1).fill(undefined).map((_, level) => /*#__PURE__*/React.createElement("svg", {
      width: blockSize,
      height: blockSize,
      key: level
  ...
  const containerStyles = {
    ...
    ...(useAnimation && {
      ...
      [`--${NAMESPACE}-loading-active`]: colorScheme === 'light' ? chroma(colorScale[0]).darken(0.3).hex() : chroma(colorScale[0]).brighten(0.25).hex()
    })
  };
